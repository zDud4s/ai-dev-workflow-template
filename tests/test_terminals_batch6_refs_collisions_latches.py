...
# ---------------------------------------------------------------------------
# Target 4 — termSendResumeChat double-send latch
# ---------------------------------------------------------------------------


def test_term_send_resume_chat_has_in_flight_latch():
    """The disabled-button guard alone is not enough: termSend() is also
    reachable via the Enter keydown handler, which does not consult
    sendBtn.disabled. Need a t._resumeInFlight latch (mirror of
    t._codexAwaitInFlight in termCodexAwaitNextTurn)."""
    body = _slice_function(_src(), "async function termSendResumeChat(")
    assert "_resumeInFlight" in body, (
        "termSendResumeChat must set t._resumeInFlight to drop overlapping "
        "calls — disabled-button guard does not catch Enter-key entries"
    )
    # The latch must be checked at the entry and released in finally.
    assert re.search(r"if\s*\(\s*t\._resumeInFlight\s*\)\s*return", body), (
        "termSendResumeChat must early-return when t._resumeInFlight is "
        "already true (overlapping caller wins)"
    )
    assert re.search(r"t\._resumeInFlight\s*=\s*true", body), (
        "termSendResumeChat must set the latch on entry"
    )
    assert re.search(r"t\._resumeInFlight\s*=\s*false", body), (
        "termSendResumeChat must release the latch in finally"
    )


def test_term_send_resume_chat_disables_send_button_before_trim():
    """REGRESSION — the existing pre-trim disable must remain so a
    synchronous re-entry sees a disabled button on the microtask boundary."""
    body = _slice_function(_src(), "async function termSendResumeChat(")
    # First sendBtn.disabled = true happens before the (text || "").trim() check.
    disable_idx = body.find("sendBtn.disabled = true")
    trim_idx = body.find(".trim()")
    assert disable_idx != -1 and trim_idx != -1
    assert disable_idx < trim_idx, (
        "REGRESSION: send button must be disabled BEFORE the trimmed-empty "
        "guard so a fast double-click that lands on the microtask "
        "boundary sees the disabled state and bails"
    )


def test_term_send_resume_chat_latch_released_before_button_reenable():
    """The latch release must happen before re-enabling the button so a
    click that lands the instant the button paints enabled is allowed
    through (rather than being dropped by a stale latch)."""
    # Strip comments + string literals first; the explanatory comment in the
    # finally block contains the literal "t.sendBtn.disabled = false" token,
    # which would otherwise win the .find() race.
    body = _strip_comments_and_strings(
        _slice_function(_src(), "async function termSendResumeChat(")
    )
    fin = body.rfind("finally")
    assert fin != -1, "no finally block in termSendResumeChat"
    tail = body[fin:]
    latch_idx = tail.find("_resumeInFlight = false")
    btn_idx = tail.find("sendBtn.disabled = false")
    assert latch_idx != -1, "no latch release in finally"
    assert btn_idx != -1, "no sendBtn re-enable in finally"
    assert latch_idx < btn_idx, (
        "Release t._resumeInFlight BEFORE re-enabling sendBtn so the next "
        "click sees a consistent state — otherwise a click between the "
        "two writes would still be dropped by the latch"
    )
