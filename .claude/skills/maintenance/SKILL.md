...
6. Scan TODOs — call `todos_parser.scan_and_append(repo)` then `todos_parser.auto_resolve(repo)` (via Bash if available). Capture `[followup]` lines in memory, `## Follow-ups` in latest plan Handoff, and `TODO|FIXME|XXX` in diffs since last maintenance commit. Auto-resolve only SUGGESTS (`status="resolved-suggested"`), never closes. Writes: `.ai/local/ledgers/todos.jsonl`, `.ai/TODO.md`, `.ai/.todos.lock`, `.ai/local/.todos-parser.log`.
